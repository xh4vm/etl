...
class ElasticsearchLoader:
    ...
    def __init__(self, settings : ElasticsearchSettings, state : BaseState, es_conn : Optional[Elasticsearch] = None, chunk_size : int = 100) -> None:
    ...
    def _reconnection(self) -> Elasticsearch:
        ...
        return Elasticsearch([f'{self._settings.protocol}://{self._settings.user}:{self._settings.password}@{self._settings.host}:{self._settings.port}'])
    ...
    def _load_wrapper(self, data: Iterator[tuple[FilmWorkModelIntoES, str]], index : str) -> Iterator[FilmWorkModelIntoES]:
        down_limit = None
        i = 0
        ...
        for movie, updated_at in data:
            ...
            # не нашел другого способа переопределения _id
